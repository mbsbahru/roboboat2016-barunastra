...
public class SeekBarVal41 extends Activity implements OnTouchListener, CvCameraViewListener2 {
	...
	private static final int    VIEW_MODE_HSV31   = 3;
	private static final int    VIEW_MODE_HSV41   = 0;
	private static final int    VIEW_MODE_BALIK   = 4;
...
//		           Log.i(TAG, "OpenCV loaded successfully");  
					...
					mOpenCvCameraView.setOnTouchListener(SeekBarVal41.this);
				} break;
				default:
				{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.biner41);
...
//		mItemExposure = menu.add("exp");
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		Imgproc.putText(mRgba, "Interop", new Point (mRgba.cols() / 2, mRgba.rows() * 0.15), Imgproc.FONT_HERSHEY_SIMPLEX, 0.7, new Scalar(0, 0, 255), 2);

//		if (viewMode==VIEW_MODE_ASLI)return mRgba;
		if (viewMode==VIEW_MODE_HSV31){
			onCameraViewStopped();
			Intent intent = new Intent(SeekBarVal41.this, SeekBarVal31.class);
			startActivity(intent);
			finish();
		}
		else if (viewMode==VIEW_MODE_HSV21){
			onCameraViewStopped();
			Intent intent = new Intent(SeekBarVal41.this, SeekBarVal21.class);
			startActivity(intent);
			finish();
		}
		else if (viewMode==VIEW_MODE_HSV11){
			onCameraViewStopped();
			Intent intent = new Intent(SeekBarVal41.this, SeekBarVal11.class);
			startActivity(intent);
			finish();
		}
		else if (viewMode==VIEW_MODE_BALIK){
			onCameraViewStopped();
			Intent intent = new Intent(SeekBarVal41.this, BluetoothConf.class);
			startActivity(intent);
			finish();
		}
