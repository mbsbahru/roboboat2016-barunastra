...
//		mItemExposure = menu.add("exp");
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		if (viewMode==VIEW_MODE_HSV11){
			...
			seekHmin2= findViewById(R.id.min);
			seekHmax2= findViewById(R.id.max);
			seekSmin2= findViewById(R.id.min);
			seekSmax2= findViewById(R.id.max);
			seekVmin2= findViewById(R.id.min);
			seekVmax2= findViewById(R.id.max);
...
//            Imgproc.fillPoly(mRgba, contours, new Scalar((mBlobColorRgba.val[0]+63>255)?mBlobColorRgba.val[0]-63:mBlobColorRgba.val[0]+63, (mBlobColorRgba.val[1]+63>255)?mBlobColorRgba.val[1]-63:mBlobColorRgba.val[1]+63, (mBlobColorRgba.val[2]+63>255)?mBlobColorRgba.val[2]-63:mBlobColorRgba.val[2]+63));
				...
				Imgproc.putText(mRgba, "valBot:   " + Hmin2 + ",  " + Smin2 + ",  " + Vmin2 + ",  " + Erode2, new Point (mRgba.cols() / 8, mRgba.rows() * 0.05), Imgproc.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(0, 255, 0), 1);
				Imgproc.putText(mRgba, "valMax:   " + Hmax2 + ",  " + Smax2 + ",  " + Vmax2 + ",  " + Dilate2, new Point (mRgba.cols() / 8, mRgba.rows() * 0.1), Imgproc.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(0, 255, 0), 1);
...
//			nLux2 = event.values[1];
	...
	float yRoll;
	float zAzim;
	// Sensor event listener for orientation
	public class MySensorEventListener implements SensorEventListener {
		...
		@Override
		public void onAccuracyChanged(Sensor sensor, int accuracy) {
			// Implement accuracy changes handling if needed
		}
	}
